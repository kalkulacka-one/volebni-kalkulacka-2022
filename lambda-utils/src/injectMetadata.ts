...
const handler: Handler = async (event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
  ...
  metaInDocument.forEach((child) => {
    if (domutils.hasAttrib(child, "property") && domutils.getAttributeValue(child, "property") === "og:image") {
      domutils.replaceElement(child, htmlparser2.parseDocument('<meta property="og:image" content="https://kalkulacka.ceskodigital.net/image/${uuid}" />'))
    }
    if (domutils.hasAttrib(child, "property") && domutils.getAttributeValue(child, "property") === "og:image:secure_url") {
      domutils.replaceElement(child, htmlparser2.parseDocument('<meta property="og:image" content="https://kalkulacka.ceskodigital.net/image/${uuid}" />'))
    }
  });
